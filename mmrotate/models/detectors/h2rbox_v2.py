...
class H2RBoxV2Detector(SingleStageDetector):
    ...
    def rotate_crop(
        ...
        if rot != 0:
            ...
            grid = grid.reshape(-1, 2).matmul(tf).view(n, h, w, 2) # 计算旋转后的坐标
            # rotate
    ...
    def loss(self, batch_inputs: Tensor,
        ...
        offset = 1 # 原始视图的 bid 小数部分为 0.2  
        for gt_instances in batch_gt_instances: # gt_instances 为一幅图像中的所有实例
            gt_instances.bid = torch.arange(
                0,
                len(gt_instances.bboxes), # len() 可获取对于图像中实例数
                1,
                device=gt_instances.bboxes.device) + offset + 0.2
                # 图中有 3 个实例 -> gt_instances.bid = [1.2, 2.2, 3.2]
            offset += len(gt_instances.bboxes) 
        # 循环后 gt_instances = [1.2, 2.2, 3.2, 4.2, ...] -> 所有图像实例的汇总

        # 旋转视图生成
        rot = math.pi * ( # 生成 π/4 ~ 3π/4 的随机旋转角度
        ...
        batch_inputs_rot, batch_gt_rot = self.rotate_crop( # 旋转视图+中心裁切
            batch_inputs, rot, self.crop_size, batch_gt_rot, self.padding)
        offset = 1 # 旋转视图的 bid 小数部分为 0.4
